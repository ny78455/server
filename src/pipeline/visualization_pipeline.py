...
class RunVisPipeline:
    ...
    def predict(self, MF=1.0):
        try:
            ...
            EX = pd.read_csv(os.path.join('artifacts', 'df_cleaned.csv'))
    ...
    def preprocess_and_visualize(concatenated_df, x=None, y=None):
        try:
            ...
            concatenated_df_new = concatenated_df_new.drop(columns=['EMASignal', 'isPivot',
       ...
       'Candle_direction'])
            ...
            '''def pointpos(x):
                if x['Prediction'] != 0:
                    ...
                    return np.nan'''
            concatenated_df_new['pointpos'] = concatenated_df_new['Low'] + 0.5e-3

            #concatenated_df_new['pointpos'] = concatenated_df_new.apply(lambda row: pointpos(row), axis=1)
            concatenated_df_new['Prediction_plot'] = np.where(concatenated_df_new['Prediction'] // 10 == 2, concatenated_df_new['Prediction'] % 10, 1)

            concatenated_df_new.to_csv('check1.csv')
            concatenated_df_new.to_csv(os.path.join('templates','check1.csv'))
